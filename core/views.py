...
def submit_login(request):
    if request.POST:
        ...
        if usuario is not None:
            ...
            messages.error(request, "Ususario ou senha inválido")
    return redirect('/')
...
def evento(request):
    ...
    dados = {}
    if id_evento:
        dados ['evento'] = Eventos.objects.get(id_evento)
...
def json_lista_evento(request, id_usuario):
    ...
    evento = Eventos.objects.filter(usuario=usuario).values('id', 'titulo')
